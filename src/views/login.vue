<template>
  <div class="login-wrap">
    <el-form class="login-container">
      ...
      <router-link to="/show">show database detail</router-link>

      <el-form-item >
        <el-input type="text" v-model="username_"  placeholder="用户账号" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item >
        <el-input type="password" v-model="password_" placeholder="用户密码" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item>
...
export default{
  ...
  methods:{
    Login:function(){
      ...
      axios.post('http://localhost:8080/doLogin',qs.stringify(data))
          .then(function (response) {
            if(response.data == 'You Fucked Up'){
              alert("密码根本就不对啊👿");
            }else{
              if(response.data.map.type=='student'){
                router.push({

                  path:'/student',
                  query: {
                    user:JSON.stringify(response.data.map.user)
                  }
                })
              }
              if(response.data.map.type=='teacher'){
                router.push({
